...
const EnhancedStudentTable = () => {
    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        if (res.data.users.length > 0) {
          ...
          console.log(
            "Students - Interest IDs:",
            res.data.users[0].interest_ids
          );
          console.log(
            "Students - Interest List:",
            res.data.users[0].interestList
          );
        }
  ...
  const handleSort = (field: keyof Student) => {
    ...
    if (newDirection) {
      trackEvent("Active Students - Table Sorted", {
        sort_field: field,
        sort_direction: newDirection,
        previous_field: sortField,
      });
      console.log("✅ Active Students - Table Sorted", {
        field,
  ...
  const handleCopyId = async (id: string) => {
    try {
      ...
      trackEvent("Active Students - User ID Copied", {
        user_id: id,
      });
      console.log("✅ Active Students - User ID Copied", { user_id: id });
      setCopiedId(id);
  ...
  const exportToCSV = () => {
    ...
    console.log("✅ Active Students - Data Exported", {
  ...
  return (
    <div className="enhanced-student-table">
        ...
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
          ...
          <CInputGroup className="search-input" style={{ maxWidth: "400px" }}>
            ...
            <CFormInput
              ...
              onChange={(e) => {
                ...
                console.log("✅ Active Students - Search Input", {
          ...
          <div className="d-flex gap-2">
            ...
            <CBadge color="info" className="d-flex align-items-center px-3">
              ...
              {processedStudents.length === 1 ? "student" : "students"}
            </CBadge>
          </div>
        </div>
      </div>
      ...
      {/* Table */}
      <div
        className="table-wrapper border border-top-0"
            ...
            {paginatedStudents.length === 0 ? (
              ...
              paginatedStudents.map((student) => (
                <CTableRow
                  key={student.id}
                  color={
                    flaggedUsers.includes(student.id) ? "danger" : undefined
                  }
                >
                  <CTableDataCell>
                    <CTooltip
                    ...
                    >
                      ...
                      >
                        ...
                        <CIcon
                          icon={
                            copiedId === student.id ? cilCheckCircle : cilCopy
                          }
                          size="sm"
                          style={{
                            color:
                              copiedId === student.id ? "green" : "#6c757d",
                          }}
                        />
                      </div>
                    </CTooltip>
                  ...
                  <CTableDataCell>
                    <div
                      ...
                      style={{
                        maxWidth: "200px",
                        cursor:
                          student.interests.length > 0 ? "pointer" : "default",
                      }}
                      onClick={() =>
                        student.interests.length > 0 &&
                        handleShowInterests(student.name, student.interests)
                      }
                    >
                      {student.interests.length > 0 ? (
                        <>
                    ...
                    <CDropdown alignment="end">
                      ...
                      <CDropdownMenu>
                        <CDropdownItem
                          ...
                          onClick={() => {
                            trackEvent(
                              "Active Students - View Profile Action",
                              {
                        ...
                        <CDropdownItem
                          href="#"
                          onClick={() => {
                            trackEvent("Active Students - Send Email Action", {
                              student_id: student.id,
                              student_name: student.name,
                              student_email: student.email,
                            });
                            console.log(
                              "✅ Active Students - Send Email Action",
                              { email: student.email }
                            );
                          }}
                        >
                        ...
                        <CDropdownItem
                          onClick={() => {
                            const alreadyFlagged = flaggedUsers.includes(
                              student.id
                            );
                            trackEvent("Active Students - Flag User Action", {
                              student_id: student.id,
                              student_name: student.name,
                              already_flagged: alreadyFlagged,
                            });
                            console.log(
                              "✅ Active Students - Flag User Action",
                              { id: student.id, flagged: !alreadyFlagged }
                            );
                            handleToggleFlagUser(student.id);
                          }}
                        >
                        ...
                        <CDropdownItem
                          href="#"
                          onClick={() => {
                            trackEvent("Active Students - Edit User Action", {
                              student_id: student.id,
                              student_name: student.name,
                            });
                            console.log(
                              "✅ Active Students - Edit User Action",
                              { id: student.id }
                            );
                          }}
                        >
                        ...
                        <CDropdownItem
                          ...
                          onClick={() => {
                            trackEvent(
                              "Active Students - Suspend User Action",
                              {
                                student_id: student.id,
                                student_name: student.name,
                              }
        ...
        <div className="pagination-wrapper p-3 bg-light rounded-bottom border border-top-0">
          <div className="d-flex justify-content-between align-items-center">
            <div className="text-muted">
              ...
              of {processedStudents.length} entries
            </div>
            <CPagination aria-label="Page navigation">
              <CPaginationItem
                ...
                onClick={() => {
                  ...
                  trackEvent("Active Students - Page Changed", {
                    previous_page: currentPage,
                    new_page: newPage,
                    total_pages: totalPages,
                    total_entries: processedStudents.length,
                  });
                  console.log("\u2705 Active Students - Page Changed", {
                    from: currentPage,
                    to: newPage,
                  });
      ...
      >
          ...
          {selectedUserInterests && (
            <div>
              <div className="mb-3">
                ...
                {selectedUserInterests.interests.length}
              </div>
              <CRow className="g-3">
        ...
        <CModalFooter>
          <CButton
            color="secondary"
            onClick={() => setInterestsModalVisible(false)}
          >
            Close
          </CButton>
        </CModalFooter>
