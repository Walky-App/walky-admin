...
export default function UploadArea({ label, name, path, required }) {
  ...
  return (
    ...
    <label
      htmlFor={name}
      className="block text-sm font-bold leading-6 text-zinc-900">
      {label}
    </label>
    <label
      htmlFor={name}
      ...
      <div className="mt-2 flex justify-center rounded-lg bg-zinc-100 px-6 py-10">
        <div className="text-center">
          ...
          <div className="mt-4 flex-col text-sm leading-6 text-zinc-600">
            ...
            <input id={name} required={required} onChange={handleFileUpload} name={name} type="file" accept="image/*,application/pdf" className="sr-only" />
