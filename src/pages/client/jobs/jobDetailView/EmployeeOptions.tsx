...
export const EmployeeOptions = ({ potentialApplicants, shiftId, setJob, jobId }: EmployeeOptionsProps) => {
    ...
    const addEmployeeShift = async () => {
        try {
            const response = await requestService({ path: `shifts/add-one/${shiftId}`, method: 'PATCH', body: JSON.stringify({ userId: applicantId, jobId }) })
    ...
    return (
        <div className="flex h-12 justify-center space-x-2 md:justify-end">
            <Dropdown
                ...
                filter
            />
            <Button type="submit" onClick={addEmployeeShift}>Add <PlusIcon className='w-7 h-7' /> </Button>
        </div>
    )
}
