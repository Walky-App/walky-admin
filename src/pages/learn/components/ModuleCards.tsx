...
export default function ModuleCards({ module, filter = '', isLoading = true }: ModuleCardsProps) {
    ...
    const modulesFilter = () => {
        ...
        return modulesTemp.filter(module =>
            module.title.toLowerCase().includes(filter.toLocaleLowerCase()),
        )
    }
    return (
        <>
                        ...
                        module.length !== 0 ?
                                ...
                                {modulesFilter().map(module => (
                                    <div key={module._id} className="mb-4 flex h-30 rounded-2xl border border-zinc-100 bg-white sm:h-32" >
                                        <div className="m-3">
                                            {
                                                module.image ?
                                                    <img alt={`Hemp Temp ${module.title} module`} src={module.image} className="h-full w-36 rounded-xl" />
                                                    :
                                        ...
                                        <div className="m-3 flex flex-1 flex-col justify-center gap-3">
                                            ...
                                            <div className="inline-flex h-5 w-full items-center justify-start gap-2">
                                                <div className="flex items-center justify-start gap-1">
                                                    ...
                                                    <div className="flex items-center h-5 text-xs font-medium text-black">{secondsToHours(module.total_time)} Time</div>
                                                </div>
                                                <div className="h-1 w-1 rounded-full bg-stone-500"></div>
                                                <div className="flex items-center justify-start gap-1">
                                                    <NewspaperIcon className="h-5" />
                                                    <div className="flex items-center h-5 text-xs font-medium text-black">{module.units?.length} Units</div>
                                                </div>
                                                <div className="h-1 w-1 rounded-full bg-stone-500"></div>
                                                <div className="flex items-center justify-start gap-1">
                                            ...
                                            <div className="h-12 text-xs font-normal text-stone-500">Investigate the cultivation and processing of cannabis, starting from seeds to a diversity of end products...</div>
                                        </div>
                            ...
                            <>
                                <div className="flex flex-col items-center justify-center h-96">
                                    ...
                                    <div className="text-sm font-normal text-stone-500">We are working on the modules for you, coming soon</div>
                                </div>
                            </>
                    }</> : <div className="flex flex-col items-center justify-center h-96">
