...
export const VideoPlayer = ({ url, captions, title }: VideoPlayerProps) => {
  ...
  const currentTime: number | string = videoPlayerRef.current ? videoPlayerRef.current.getCurrentTime() : '00:00'
  const duration: number | string = videoPlayerRef.current ? videoPlayerRef.current.getDuration() : '00:00'
  ...
  const handleFastFoward = useCallback(() => {
    videoPlayerRef.current?.seekTo(videoPlayerRef.current.getCurrentTime() + 10)
  }, [videoPlayerRef])
  ...
  }, [playPauseHandler, handleFastFoward, rewindHandler])
  ...
  const seekHandler = (event: SliderChangeEvent) => {
    const value = event.value as number
    if (value !== undefined) {
      videoPlayerRef.current?.seekTo(parseFloat((value / 100).toString()))
      setVideoState({ ...videoState, played: parseFloat((value / 100).toString()) })
    }
  }
  const seekMouseUpHandler = (event: SliderChangeEvent) => {
    const value = event.value as number
    if (value !== undefined) {
  ...
  const volumeChangeHandler = (event: SliderChangeEvent) => {
    const value = event.value as number
  ...
  const volumeSeekUpHandler = (event: SliderChangeEvent) => {
    const value = event.value as number
    if (value !== undefined) {
      const newVolume = parseFloat(value.toString()) / 100
  ...
  const progressHandler = (state: OnProgressProps) => {
    if (count > 3) {
      controlRef.current?.style.setProperty('visibility', 'hidden')
  ...
  const mouseMoveHandler = () => {
    controlRef.current?.style.setProperty('visibility', 'visible')
    setCount(0)
  }
  ...
  return (
    <div ref={playerContainerRef} className="relative bg-black" onMouseMove={mouseMoveHandler}>
      <ReactPlayer
        ref={videoPlayerRef}
        url={url}
        width="100%"
...
const Control = ({
  ...
  mute: boolean
  played: number
  onSeek: (event: SliderChangeEvent) => void
  onMute: () => void
  onVolumeChangeHandler: (event: SliderChangeEvent) => void
  onVolumeSeekUp: (event: SliderChangeEvent) => void
  onSeekMouseUp: (event: SliderChangeEvent) => void
  volume: number
  duration: string
  currentTime: string
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-1 items-center justify-between px-2 py-3">
            <div className="flex items-center  gap-2">
              <button onClick={onRewind} type="button">
                <svg
                  width="30"
                  height="30"
                  viewBox="0 0 24 24"
              ...
              <button className="ml-3 mr-2" onClick={onMute} type="button">
                {mute ? (
                  <svg
                    width="30"
                    height="30"
              ...
              <Slider
                className="mr-2 h-2 min-w-16"
                onChange={onVolumeChangeHandler}
                value={volume * 100}
                onSlideEnd={onVolumeSeekUp}
              />
            </div>
