...
// Mock the actual service object, not the module
...
describe("Walky Admin - CampusSync Component", () => {
  ...
  it("displays header: Campus Sync Management", async () => {
    ...
    // Wait for campus data to render
    await waitFor(() => {
  ...
  it("fetches and displays campus rows with key info and action buttons", async () => {
    ...
    // Action buttons
    const syncButtons = screen.getAllByTestId("button-sync");
  ...
  it("handles Sync Campus button: triggers sync, shows spinner, updates status, and displays alert", async () => {
    ...
    // Set up both responses: before and after sync refetch
    ...
    // Wait for success alert and mutation call
    await waitFor(() => {
  ...
  it("opens Preview Modal on button click and displays boundary metadata", async () => {
    ...
    // Use `within` to scope to modal only
    ...
    expect(
        modalUtils.getAllByText((_, node) =>
  ...
  it("opens Logs Modal on button click and displays sync logs", async () => {
    ...
    expect(modalUtils.getByText("Date")).toBeInTheDocument();
    expect(modalUtils.getByText("Campus")).toBeInTheDocument();
    expect(modalUtils.getByText("Status")).toBeInTheDocument();
    expect(modalUtils.getByText("Added")).toBeInTheDocument();
    expect(modalUtils.getByText("Updated")).toBeInTheDocument();
    expect(modalUtils.getByText("Removed")).toBeInTheDocument();
    expect(modalUtils.getByText("Duration")).toBeInTheDocument();
  
    // Validate one log row
    expect(modalUtils.getByText("Test University")).toBeInTheDocument();
  ...
  it("displays a loading spinner while campuses are being fetched", async () => {
    mockedService.getCampusesWithSyncStatus.mockImplementation(() => new Promise(() => {})); // never resolves
  
    renderWithProviders(<CampusSync />);
  
    expect(await screen.findByRole("status")).toBeInTheDocument(); // CoreUI uses this for CSpinner
  });
