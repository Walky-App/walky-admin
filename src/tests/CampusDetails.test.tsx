  ...
  it("pre-fills form inputs with campusData from navigation state", async () => {
    ...
    describe("handles form input changes", () => {
        ...
        it("updates dawn and dusk times", async () => {
          ...
          // Clear and change with userEvent
            ...
            dawnTimeInput.blur(); // Trigger state update
        ...
        it("updates ambassador multi-select dropdown", async () => {
            ...
            // Mock the ambassador service
            const mockedAmbassadorService =
              ambassadorModule.ambassadorService as jest.Mocked<typeof ambassadorModule.ambassadorService>;
            mockedAmbassadorService.getAll.mockResolvedValueOnce(mockAmbassadors);
          
            // Render the component with pre-filled campus data
            renderWithProviders(<CampusDetails />, { state: { campusData: mockCampusData } });
          
            // Wait for the dropdown label to appear to confirm ambassadors loaded
            await screen.findByText(/student ambassadors/i);
          
            // Now get the dropdown toggle
            ...
            // Optional: check that input value updated
            expect(input).toHaveValue("alice");
          
            // Press Enter to select the filtered option
            await userEvent.keyboard("{Enter}");
          
            // Optional: screen.debug(); // to inspect rendered DOM if needed
          });          
    ...
    it("renders Campus Boundary and triggers onBoundaryChange when edited", async () => {
        ...
        // Step 1: Find the boundary container
        const mapContainer = await screen.findByTestId("campus-boundary-container");
        expect(mapContainer).toBeInTheDocument();
      
        // Step 2: Confirm it renders placeholder boundary content — no need to query Leaflet internals
        // Optional: assert height or other container style if needed
        expect(mapContainer).toHaveStyle({ height: "800px" });
      
        // Step 3: (Optional) Simulate a boundary update via the prop
        // Since we cannot interact with the actual map, it's best to unit test CampusBoundary separately
      });    
      ...
      it("validates required fields and disables save if polygon or fields are missing", async () => {
        ...
        // ❌ Save should remain disabled if required fields or polygon are missing
        expect(saveButton).toBeDisabled();
      
        // ✅ Optionally, assert that specific fields are still empty
      ...
      it("calls createCampusMutation.mutate when creating a new campus", async () => {
        ...
        // 🔧 Spy before render so mutation hooks use this
